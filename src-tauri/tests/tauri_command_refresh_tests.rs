...
mod tauri_command_refresh_tests {
    ...
    impl MockAudioState {
        async fn new() -> Self {
            let device_manager =
                AudioDeviceManager::new().expect("Failed to create device manager");
    ...
    async fn test_refresh_vs_enumerate_consistency() {
        ...
        for device in &enumerate_devices {
            ...
            assert!(
                found,
                "Device {} should be present in both enumerate and refresh",
                device.id
            );
        }
    }
    ...
    async fn test_concurrent_refresh_calls() {
        ...
        for (i, result) in results.into_iter().enumerate() {
            ...
            assert!(
                device_result.is_ok(),
                "Concurrent refresh call {} should succeed",
                task_id
            );
            ...
            assert!(
                !devices.is_empty(),
                "Concurrent call {} should return devices",
                task_id
            );
        }
    }
    ...
    async fn test_refresh_command_serialization() {
        ...
        for device in devices {
            ...
            assert_eq!(
                device.id, deserialized_device.id,
                "Device ID should survive serialization"
            );
            assert_eq!(
                device.name, deserialized_device.name,
                "Device name should survive serialization"
            );
        }
    }
    ...
    async fn test_refresh_command_with_empty_system() {
        ...
        match result {
            Ok(devices) => {
                ...
                assert!(
                    !devices.is_empty(),
                    "Most systems should have at least one audio device"
                );
            }
            Err(_error) => {
                ...
                println!(
                    "System appears to have no audio devices - this can happen in CI environments"
                );
            }
        }
    }
    ...
    async fn test_refresh_command_output_structure() {
        ...
        for device in devices {
            ...
            assert!(
                !device.supported_channels.is_empty(),
...
mod refresh_error_tests {
    ...
    async fn test_refresh_with_system_changes() {
        ...
        let device_manager = AudioDeviceManager::new().expect("Device manager should be created");
