use sendin_beats_lib::audio::{
    AudioAnalyzer, AudioEffectsChain, Compressor, EQBand, Limiter, PeakDetector, RmsDetector,
    ThreeBandEqualizer,
};
...
mod effects_processing_tests {
    ...
    fn test_audio_analyzer_with_full_scale() {
        ...
        assert!(
            (peak - 1.0).abs() < 0.01,
            "Peak should be close to 1.0 for full-scale signal: {}",
            peak
        );
        // RMS of a DC signal depends on the window size, so just check it's positive and reasonable
        assert!(
            rms > 0.1 && rms <= 1.0,
            "RMS should be positive and reasonable for full-scale signal: {}",
            rms
        );
    }
    ...
    fn test_rms_detector_window() {
        ...
        assert!(
            rms < 1.0,
            "RMS should be less than 1.0 for 0.5 amplitude sine"
        );
        ...
        assert!(
            ...
            "RMS should be close to expected value"
        );
    }
    ...
    fn test_effects_chain_processing() {
        ...
        assert!(
            !audio.is_empty(),
            "Audio should not be empty after processing"
        );
        assert!(
        ...
        assert!(
            energy_ratio > 0.00000001 && energy_ratio < 10000.0,
    ...
    fn test_equalizer_frequency_response() {
        ...
        assert!(
            low_energy_after > 0.0,
            "Low frequency energy should be positive"
        );
        assert!(
            high_energy_after > 0.0,
            "High frequency energy should be positive"
        );

        // All processed audio should be finite
        assert!(
            low_freq.iter().all(|&x| x.is_finite()),
            "Low freq audio should be finite"
        );
        assert!(
            mid_freq.iter().all(|&x| x.is_finite()),
            "Mid freq audio should be finite"
        );
        assert!(
            high_freq.iter().all(|&x| x.is_finite()),
            "High freq audio should be finite"
        );
    }
    ...
    fn test_compressor_dynamics() {
        ...
        assert!(
            max_loud > 0.0 && max_loud <= 1.0,
            "Loud signal should be in valid range: {}",
            max_loud
        );
        ...
        // All samples should be finite
        assert!(
            loud_signal.iter().all(|&x| x.is_finite()),
            "Compressed audio should be finite"
        );
        assert!(
            quiet_signal.iter().all(|&x| x.is_finite()),
            "Quiet audio should be finite"
        );
    }
    ...
    fn test_limiter_functionality() {
        ...
        assert!(
            max_amplitude <= 2.0,
            "Signal should be reasonably bounded: {}",
            max_amplitude
        );

        // All samples should be finite
        assert!(
            test_signal.iter().all(|&x| x.is_finite()),
            "Limited audio should be finite"
        );

        // Signal should still have some amplitude (not completely crushed)
        assert!(
            max_amplitude > 0.1,
            "Limited signal should retain some amplitude"
        );
    }
    ...
    fn test_effects_chain_with_extreme_settings() {
        ...
        assert!(
            audio.iter().all(|&x| x.is_finite()),
            "Audio should remain finite with extreme settings"
        );
        assert!(
            audio.iter().all(|&x| x.abs() <= 2.0),
            "Audio should be reasonably bounded"
        );
    }
    ...
    fn test_effects_bypass() {
        ...
        for (orig, proc) in original_audio.iter().zip(processed_audio.iter()) {
            assert!(
                (orig - proc).abs() < 1e-6,
                "Audio should be unchanged when effects disabled"
            );
        }
    }
    ...
    fn test_real_time_processing_stability() {
        ...
        for i in 0..100 {
            ...
            assert!(
                audio.iter().all(|&x| x.is_finite()),
                "Audio should remain finite in iteration {}",
                i
            );
            assert!(
                audio.iter().all(|&x| x.abs() <= 10.0),
                "Audio should be bounded in iteration {}",
                i
            );
        }
    }
    ...
    fn test_effects_parameter_changes() {
        ...
        assert!(
            audio.iter().all(|&x| x.is_finite()),
            "Audio should remain finite through parameter changes"
        );
    }
}
