...
mod device_validation_tests {
    ...
    async fn test_device_id_invalid_characters() {
        ...
        for invalid_id in invalid_ids {
            ...
            assert!(
                ...
                "Invalid device ID '{}' should show character validation error",
                invalid_id
            );
        }
    }
        ...
        for valid_id in valid_ids {
            ...
            assert!(
                !error_msg.contains("invalid characters")
                    && !error_msg.contains("Device ID cannot be empty"),
                "Valid device ID '{}' should not fail validation: {}",
                valid_id,
                error_msg
            );
        }
    ...
    async fn test_output_device_id_validation() {
        ...
        assert!(
            result.is_err(),
            "Empty output device ID should fail validation"
        );
        assert!(result
            .unwrap_err()
            .to_string()
    ...
    async fn test_output_device_id_too_long() {
        ...
        assert!(
            result.is_err(),
            "Long output device ID should fail validation"
        );
        assert!(result
            .unwrap_err()
            .to_string()
    ...
    async fn test_device_id_length_boundary_conditions() {
        ...
        let error_msg = result.unwrap_err().to_string();
        assert!(
            !error_msg.contains("Device ID too long"),
            "256-character device ID should pass length validation"
        );
        ...
        assert!(
            result.is_err(),
            "257-character device ID should fail validation"
        );
        assert!(result
            .unwrap_err()
            .to_string()
    ...
    async fn test_device_id_edge_cases() {
        ...
        assert!(
            !error_msg.contains("invalid characters"),
            "Underscore and dash combination should be valid"
        );
    }
}
