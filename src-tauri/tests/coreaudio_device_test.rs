...
async fn test_coreaudio_device_selection() {
    ...
    let device_manager =
        std::sync::Arc::new(AudioDeviceManager::new().expect("Failed to create device manager"));
    ...
    let hardware_devices: Vec<_> = devices
        .iter()
        .filter(|d| d.host_api == "CoreAudio (Direct)" && d.is_output)
        .collect();

    println!(
        "Found {} CoreAudio hardware output devices:",
    ...
    for device in &hardware_devices {
        ...
        match device_manager.find_audio_device(&device.id, false).await {
            Ok(device_handle) => {
                ...
                let mixer = VirtualMixer::new_with_device_manager(config, device_manager.clone())
                    .await
                ...
                match mixer.set_output_stream(&device.id).await {
                    Ok(()) => {
                        println!(
                            "  ✅ Successfully set as output stream (placeholder implementation)"
                        );
                    }
                    Err(e) => {
    ...
    assert!(
        ...
        "Should find at least one CoreAudio hardware device"
    );
}
