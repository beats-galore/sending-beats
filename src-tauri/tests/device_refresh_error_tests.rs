use sendin_beats_lib::audio::{AudioDeviceManager, VirtualMixer, MixerConfig};
...
mod device_refresh_error_tests {
            ...
            match result {
                ...
                Err(error) => {
                    ...
                    assert!(!error.to_string().is_empty(), 
                        "Error {} should have a message", i);
                    println!("Call {}: Error (expected in stress test): {}", i, error);
                }
            }
        }
        
        println!("Stress test results: {} successes, {} errors", success_count, error_count);
        // At least some calls should succeed
        assert!(success_count > 0, "At least some rapid calls should succeed");
    }
    ...
    async fn test_device_manager_with_invalid_state() {
        ...
        for i in 0..5 {
            ...
            match result {
                Ok(devices) => {
                    assert!(!devices.is_empty(), "Should find devices on iteration {}", i);
                }
                Err(error) => {
                    ...
                    println!("Enumeration {} failed (might be expected): {}", i, error_msg);
                }
            }
        }
    }
    ...
    async fn test_concurrent_device_access_safety() {
        ...
        for (i, task_result) in results.into_iter().enumerate() {
            ...
            match enum_result {
                ...
                Err(error) => {
                    println!("Task {} failed (may be expected under contention): {}", task_id, error);
                }
            }
        }
    }
    ...
    async fn test_device_cache_consistency_under_stress() {
        ...
        for i in 0..50 {
            ...
            assert!(cached_device.is_some(), 
                "Device {} should remain in cache after enumeration {}", first_device_id, i);
        ...
        for device in devices.iter().take(3) {
            if device.is_input {
                ...
                if let Err(error) = result {
                    ...
                    assert!(!error_msg.is_empty(), "Input stream error should have message");
                    assert!(!error_msg.contains("panic"), "Should not contain panic");
                    assert!(!error_msg.contains("unwrap"), "Should not contain unwrap errors");
                }
            }
            ...
            if device.is_output {
                ...
                if let Err(error) = result {
                    let error_msg = error.to_string();
                    // Error should be descriptive and not a panic
                    assert!(!error_msg.is_empty(), "Output stream error should have message");
                    assert!(!error_msg.contains("panic"), "Should not contain panic");
                    assert!(!error_msg.contains("unwrap"), "Should not contain unwrap errors");
                }
            }
    ...
    async fn test_device_enumeration_with_system_limitations() {
            ...
            match result {
                Ok(devices) => {
                    ...
                    for device in devices {
                        assert!(!device.id.is_empty(), "Device ID should not be empty in attempt {}", i);
                        assert!(!device.name.is_empty(), "Device name should not be empty in attempt {}", i);
                        assert!(device.is_input || device.is_output, 
                            "Device should be input or output in attempt {}", i);
                    }
                }
                Err(error) => {
                    ...
                    assert!(!error.to_string().is_empty(), 
                        "Failed enumeration {} should have error message", i);
                }
            }
        ...
        println!("Enumeration test: {}/{} successful, {}/{} failed", 
            successful_enumerations, enumeration_attempts,
            failed_enumerations, enumeration_attempts);
        ...
        assert!(success_rate >= 0.8, 
            "Success rate should be at least 80%, got {:.2}%", success_rate * 100.0);
    }
    ...
    async fn test_device_data_validation_errors() {
        ...
        for (i, device) in devices.iter().enumerate() {
            ...
            assert!(device.id.len() <= 256, 
                "Device {} ID length {} should not exceed 256 characters", i, device.id.len());
            assert!(!device.id.is_empty(), 
                "Device {} ID should not be empty", i);
            
            // Test device name validation
            assert!(device.name.len() <= 512, 
                "Device {} name length {} should not exceed 512 characters", i, device.name.len());
            assert!(!device.name.is_empty(), 
                "Device {} name should not be empty", i);
            
            // Test sample rate validation
            for (j, &rate) in device.supported_sample_rates.iter().enumerate() {
                assert!(rate >= 8000 && rate <= 192000, 
                    "Device {} sample rate {} at index {} should be reasonable", i, rate, j);
            }
            
            // Test channel validation
            for (j, &channels) in device.supported_channels.iter().enumerate() {
                assert!(channels >= 1 && channels <= 32, 
                    "Device {} channel count {} at index {} should be reasonable", i, channels, j);
            }
            
            // Test that device has at least one capability
            assert!(device.is_input || device.is_output, 
                "Device {} should be either input or output", i);
            
            // Test host API field
            assert!(!device.host_api.is_empty(), 
                "Device {} host API should not be empty", i);
        }
    }
    ...
    async fn test_refresh_memory_safety() {
        ...
        assert!(final_result.is_ok(), "Final enumeration should succeed after memory test");
    }
    ...
    async fn test_device_manager_drop_safety() {
        ...
        for i in 0..5 {
            ...
            match device_manager {
                    ...
                    match devices {
                        Ok(device_list) => {
                            assert!(!device_list.is_empty(), 
                                "Iteration {} should find devices", i);
                        }
                        Err(error) => {
                ...
                Err(error) => {
                    println!("Device manager creation failed in iteration {}: {}", i, error);
                }
            }
        }
        ...
        assert!(final_manager.is_ok(), "Final device manager creation should succeed");
    }
}
...
mod refresh_edge_case_tests {
            ...
            Ok(device_manager) => {
                ...
                match devices_result {
                    Ok(devices) => {
                        ...
                        println!("Audio permissions available: found {} devices", devices.len());
                    }
                    Err(error) => {
                        // Limited permissions case
                        println!("Limited audio permissions: {}", error);
                        // Should not panic, should return proper error
                        assert!(!error.to_string().is_empty(), "Permission error should have message");
                    }
                }
            }
            Err(error) => {
                // Very limited permissions - can't even create device manager
                println!("Cannot create device manager (very limited permissions): {}", error);
                assert!(!error.to_string().is_empty(), "Creation error should have message");
            }
        }
    }
        ...
        for i in 0..5 {
            ...
            match result {
                ...
                Err(error) => {
                    ...
                    assert!(!error.to_string().is_empty(), "Rapid call error should have message");
                }
            }
        }
    ...
    async fn test_empty_device_list_handling() {
        ...
        if devices.is_empty() {
            ...
            assert!(non_existent_device.is_none(), 
