...
mod stream_management_tests {
    ...
    async fn test_add_input_stream_invalid_device() {
        ...
        assert!(result.is_err(), "Adding non-existent input device should fail");
        
        // The error should indicate device not found, not validation issues
        let error_msg = result.unwrap_err().to_string();
        assert!(!error_msg.contains("invalid characters"), "Error should be about device not found");
    }
    ...
    async fn test_set_output_stream_invalid_device() {
        ...
        assert!(result.is_err(), "Setting non-existent output device should fail");
        ...
        assert!(
            ...
            "Error should indicate device or stream issue: {}", error_msg
        );
    }
    ...
    async fn test_add_and_remove_input_stream() {
        ...
        assert!(add_result.is_err(), "Adding non-existent device should fail");
        
        // But validation should have passed
        let error_msg = add_result.unwrap_err().to_string();
        assert!(!error_msg.contains("invalid characters"), "Validation should pass for valid device ID");
    ...
    async fn test_audio_metrics() {
        ...
        assert!(metrics.buffer_underruns >= 0, "Buffer underruns should be non-negative");
        assert!(metrics.buffer_overruns >= 0, "Buffer overruns should be non-negative");
    }
    ...
    async fn test_mixer_with_processing() {
        ...
        let metrics = mixer.get_metrics().await;
        assert!(metrics.sample_rate > 0, "Metrics should be available during processing");
    ...
    async fn test_concurrent_stream_operations() {
        ...
        for handle in handles {
            ...
            assert!(result.is_ok(), "Concurrent stream operations should complete");
        }
    }
    ...
    async fn test_mixer_send_command() {
        ...
        assert!(result2.is_ok(), "Sending start stream command should succeed");
        
        let command3 = MixerCommand::StopStream;
        let result3 = mixer.send_command(command3).await;
        assert!(result3.is_ok(), "Sending stop stream command should succeed");
    }
