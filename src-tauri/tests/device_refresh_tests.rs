...
mod device_refresh_tests {
    ...
    async fn test_refresh_devices_returns_list() {
        ...
        assert!(!device_list.is_empty(), "Should detect at least one audio device");
    }
    ...
    async fn test_refresh_devices_cache_behavior() {
        ...
        assert!(cached_device.is_some(), "Device should be cached after enumeration");
        ...
        assert!(cached_device_after_refresh.is_some(), "Device should remain cached after refresh");
    }
    ...
    async fn test_refresh_devices_input_output_separation() {
        ...
        let output_devices: Vec<&AudioDeviceInfo> = devices.iter().filter(|d| d.is_output).collect();
        
        // Should have at least some input and output devices
        // Note: This might not be true on all systems, so we'll just log
        println!("Found {} input devices and {} output devices", 
        ...
        for device in input_devices {
            ...
            assert!(device.is_input, "Input device {} should be marked as input", device.name);
        }
        ...
        for device in output_devices {
            ...
            assert!(device.is_output, "Output device {} should be marked as output", device.name);
        }
    }
    ...
    async fn test_refresh_devices_default_devices() {
        ...
        let default_input_devices: Vec<&AudioDeviceInfo> = devices.iter()
            .filter(|d| d.is_input && d.is_default)
            .collect();
        let default_output_devices: Vec<&AudioDeviceInfo> = devices.iter()
            .filter(|d| d.is_output && d.is_default)
            .collect();
        
        // Should have at least one default input and output device
        assert!(!default_input_devices.is_empty(), "Should have at least one default input device");
        assert!(!default_output_devices.is_empty(), "Should have at least one default output device");
        ...
        for device in default_output_devices {
            assert!(!device.name.is_empty(), "Default output device should have a name");
    ...
    async fn test_device_manager_creation() {
        ...
        assert!(devices1.is_ok(), "First manager should enumerate successfully");
        assert!(devices2.is_ok(), "Second manager should enumerate successfully");
    }
    ...
    async fn test_device_id_format() {
        ...
        for device in devices {
            ...
            assert!(device.id.len() <= 256, "Device ID should not exceed 256 characters");
...
mod refresh_integration_tests {
    ...
    async fn test_refresh_with_mixer_integration() {
        ...
        let devices = device_manager.enumerate_devices().await;
        assert!(devices.is_ok(), "Device refresh should work with active mixer");
        
        let device_list = devices.unwrap();
        assert!(!device_list.is_empty(), "Should still detect devices with active mixer");
    }
    ...
    async fn test_refresh_provides_valid_device_ids_for_mixer() {
        ...
        for device in devices.iter().take(3) { // Test first 3 devices to avoid overwhelming the test
            if device.is_input {
