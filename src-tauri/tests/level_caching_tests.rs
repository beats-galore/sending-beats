...
mod level_caching_tests {
    ...
    async fn test_concurrent_level_access() {
        ...
        let handles = (0..10)
            .map(|_| {
                ...
                tokio::spawn(async move { mixer_clone.get_channel_levels().await })
            })
            .collect::<Vec<_>>();
    ...
    async fn test_concurrent_master_level_access() {
        ...
        let handles = (0..10)
            .map(|_| {
                let mixer_clone = mixer.clone();
                tokio::spawn(async move { mixer_clone.get_master_levels().await })
            })
            .collect::<Vec<_>>();
        ...
        for handle in handles {
            ...
            assert!(
                result.is_ok(),
                "Concurrent master level access should succeed"
            );
    ...
    async fn test_level_bounds() {
        ...
        assert!(
            left_peak >= 0.0 && left_peak <= 10.0,
            "Left peak should be in reasonable range"
        );
        assert!(
            left_rms >= 0.0 && left_rms <= 10.0,
            "Left RMS should be in reasonable range"
        );
        assert!(
            right_peak >= 0.0 && right_peak <= 10.0,
            "Right peak should be in reasonable range"
        );
        assert!(
            right_rms >= 0.0 && right_rms <= 10.0,
            "Right RMS should be in reasonable range"
        );
    }
    ...
    async fn test_channel_levels_data_structure() {
        ...
        for (channel_id, (peak, rms)) in &levels {
            assert!(
                peak.is_finite(),
                "Channel {} peak should be finite",
                channel_id
            );
            assert!(
                rms.is_finite(),
                "Channel {} RMS should be finite",
                channel_id
            );
            assert!(
                *peak >= 0.0,
                "Channel {} peak should be non-negative",
                channel_id
            );
            assert!(
                *rms >= 0.0,
                "Channel {} RMS should be non-negative",
                channel_id
            );
        }
    }
    ...
    async fn test_level_access_after_mixer_start() {
        ...
        assert!(
            levels.is_empty(),
            "Channel levels should still be accessible after start"
        );

        let (left_peak, left_rms, right_peak, right_rms) = master;
        assert!(
            left_peak.is_finite(),
            "Master levels should still be accessible after start"
        );
        assert!(
            left_rms.is_finite(),
            "Master levels should still be accessible after start"
