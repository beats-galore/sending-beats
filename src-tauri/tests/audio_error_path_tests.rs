...
mod audio_error_path_tests {
    ...
    async fn test_device_disconnection_handling() {
        ...
        let cpal_result = manager.find_cpal_device("disconnected_device_12345", false).await;
        assert!(cpal_result.is_err(), "Disconnected device should return error");
        
        // Test refresh after device disconnection
        let refresh_result = manager.refresh_devices().await;
        assert!(refresh_result.is_ok(), "Refresh should handle device changes gracefully");
    }
    ...
    async fn test_audio_format_changes() {
        ...
        for rate in invalid_rates {
            ...
            assert!(config_result.is_ok(), "Config creation should not panic for rate {}", rate);
        }
        ...
        for channels in invalid_channels {
            ...
            assert!(config_result.is_ok(), "Config creation should not panic for {} channels", channels);
        }
    }
    ...
    async fn test_stream_creation_failures() {
        ...
        if let Ok(mixer) = mixer_result {
            ...
            assert!(invalid_stream_result.is_err(), "Adding invalid input stream should fail");
            
            // Test setting output stream with invalid device
            let invalid_output_result = mixer.set_output_stream("invalid_output_device_12345").await;
            assert!(invalid_output_result.is_err(), "Setting invalid output stream should fail");
        }
    }
    ...
    async fn test_memory_pressure_handling() {
        ...
        assert!(allocation_result.is_ok(), "Large buffer allocation should not panic");
        ...
        tokio::spawn(async move {
            ...
            assert!(buf.len() <= 750_000, "Buffer should be cleaned up under pressure");
        }).await.expect("Memory pressure test failed");
    }
    ...
    async fn test_concurrent_access_errors() {
        ...
        for handle in handles {
            ...
            assert!(result.is_ok(), "Lock acquisition should not timeout: {:?}", result);
        }
        
        let final_state = shared_state.lock().await;
        assert_eq!(final_state.len(), 10, "All concurrent writes should succeed");
    }
    ...
    async fn test_callback_interruption_recovery() {
        ...
        println!("Total successful callbacks: {}, Total errors: {}", total_callbacks, total_errors);
    ...
    async fn test_device_property_changes() {
        ...
        assert!(devices_result.is_ok(), "Device enumeration should handle errors gracefully");
        ...
        if let Ok(devices) = devices_result {
            for device in &devices {
                ...
                assert!(device.is_input || device.is_output, "Device should be input or output");
    ...
    async fn test_streaming_interruption() {
        ...
        assert!(received_count > 0, "Some audio data should be received before interruption");
        assert!(received_count <= 10, "Interruption should limit received data");
    }
    ...
    async fn test_graceful_shutdown() {
        ...
        assert!(shutdown_completed.load(Ordering::SeqCst), "Shutdown cleanup should complete");
    }
}