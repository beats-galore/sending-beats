...
mod device_state_management_tests {
    ...
    async fn test_device_cache_initial_state() {
        ...
        assert!(
            ...
            "First device should be cached after enumeration"
        );

        let cached = cached_device.unwrap();
        assert_eq!(cached.id, devices[0].id, "Cached device ID should match");
        assert_eq!(
            cached.name, devices[0].name,
            "Cached device name should match"
        );
    }
    ...
    async fn test_device_cache_persistence() {
        ...
        let device_info: HashMap<String, AudioDeviceInfo> =
        ...
        for device_id in device_info.keys().take(3) {
            ...
            assert!(
                cached1.is_some(),
                "Device {} should be cached (check 1)",
                device_id
            );
            ...
            assert!(
                cached2.is_some(),
                "Device {} should be cached (check 2)",
                device_id
            );
            ...
            assert_eq!(
                c1.is_input, c2.is_input,
                "Cached device input flag should be consistent"
            );
            assert_eq!(
                c1.is_output, c2.is_output,
                "Cached device output flag should be consistent"
            );
        }
    }
    ...
    async fn test_device_cache_update_behavior() {
        ...
        if cached_after_refresh.is_some() {
            ...
            assert_eq!(
                cached.id, cached_initial.id,
                "Device ID should remain consistent"
            );
            assert_eq!(
                cached.name, cached_initial.name,
                "Device name should remain consistent"
            );
            ...
            assert!(
                found_in_new_enum,
                "Device should be found in new enumeration"
            );
        } else {
            // Device was removed from system - this is also valid behavior
            println!(
                "Device {} was removed from system during test",
                first_device_id
            );
            ...
            assert!(
                !found_in_new_enum,
                "Removed device should not be in new enumeration"
            );
        }
    }
    ...
    async fn test_device_cache_size_limits() {
        ...
        for i in 0..5 {
            ...
            println!(
                "Enumeration {}: found {} unique device IDs total",
                i + 1,
                all_device_ids.len()
            );
        }
        ...
        for device_id in &all_device_ids {
            ...
            assert!(
                cached.is_some(),
                "Device {} should be accessible from cache",
                device_id
            );
        }
        ...
        for i in 0..10 {
            ...
            assert!(
                result.is_none(),
                "Fake device {} should not be in cache",
                fake_id
            );
        }
        ...
        for device_id in all_device_ids.iter().take(3) {
            ...
            assert!(
                cached.is_some(),
                "Real device {} should still be accessible",
                device_id
            );
        }
    }
    ...
    async fn test_device_state_consistency() {
        ...
        for device in devices {
            ...
            assert_eq!(
                device.is_output, cached.is_output,
                "Output flag should match"
            );
            assert_eq!(
                device.is_default, cached.is_default,
            ...
            // Arrays should match
            assert_eq!(
                device.supported_sample_rates, cached.supported_sample_rates,
                "Sample rates should match for device {}",
                device.id
            );
            assert_eq!(
                device.supported_channels, cached.supported_channels,
                "Channels should match for device {}",
                device.id
            );
        }
    }
    ...
    async fn test_concurrent_cache_access() {
        ...
        for (task_id, device_results) in results {
            for (device_id, found) in device_results {
                assert!(
                    found,
                    "Task {} should find device {} in cache",
                    task_id, device_id
                );
            }
        }
    ...
    async fn test_device_metadata_integrity() {
        ...
        for device in devices {
            ...
            assert_eq!(
                device.supported_sample_rates.len(),
                cached.supported_sample_rates.len(),
                "Sample rates array length should match for device {}",
                device.id
            );

            for (i, (&original, &cached_rate)) in device
                .supported_sample_rates
                .iter()
                .zip(cached.supported_sample_rates.iter())
                .enumerate()
            {
                assert_eq!(
                    original, cached_rate,
                    "Sample rate {} should match for device {}",
                    i, device.id
                );
            }

            assert_eq!(
                device.supported_channels.len(),
                cached.supported_channels.len(),
                "Channels array length should match for device {}",
                device.id
            );

            for (i, (&original, &cached_channels)) in device
                .supported_channels
                .iter()
                .zip(cached.supported_channels.iter())
                .enumerate()
            {
                assert_eq!(
                    original, cached_channels,
                    "Channel count {} should match for device {}",
                    i, device.id
                );
            }
        }
    ...
    async fn test_cache_invalidation_scenarios() {
        ...
        let test_device_ids: Vec<String> = devices1.iter().take(3).map(|d| d.id.clone()).collect();
        ...
        for device_id in &test_device_ids {
            ...
            assert!(
                cached.is_some(),
                "Device {} should be initially cached",
                device_id
            );
        }
        ...
        for device_id in &test_device_ids {
            ...
            match cached_after_refresh {
                ...
                None => {
                    ...
                    assert!(
                        !found_in_enum,
                        "Uncached device should not be in new enumeration"
                    );

                    println!(
                        "Device {} was properly removed from cache and enumeration",
                        device_id
                    );
                }
            }
...
mod device_state_edge_cases {
    ...
    async fn test_cache_with_duplicate_device_ids() {
        ...
        assert!(
            duplicates.is_empty(),
            "Should not have duplicate device IDs: {:?}",
            duplicates
        );
    ...
    async fn test_cache_with_empty_device_names() {
        ...
        for device in devices {
            assert!(
                ...
                "Device {} should not have empty name",
                device.id
            );
            assert!(!device.id.is_empty(), "Device should not have empty ID");

            // Cached version should also have non-empty name
            let cached = device_manager.get_device(&device.id).await.unwrap();
            assert!(
                !cached.name.is_empty(),
                "Cached device {} should not have empty name",
                device.id
            );
        }
    }
    ...
    async fn test_cache_consistency_with_device_changes() {
        ...
        let initial_device_map: HashMap<String, AudioDeviceInfo> = initial_devices
            .iter()
        ...
        let updated_device_map: HashMap<String, AudioDeviceInfo> = updated_devices
            .iter()
            .map(|d| (d.id.clone(), d.clone()))
            .collect();
        ...
        for (device_id, initial_device) in &initial_device_map {
            if let Some(updated_device) = updated_device_map.get(device_id) {
                ...
                assert_eq!(
                    cached.id, updated_device.id,
                    "Device ID should be consistent"
                );
                assert_eq!(
                    cached.name, updated_device.name,
                    "Device name should be consistent"
                );
                assert_eq!(
                    cached.is_input, updated_device.is_input,
                    "Input flag should be consistent"
                );
                assert_eq!(
                    cached.is_output, updated_device.is_output,
                    "Output flag should be consistent"
                );

                // Compare with initial to see what changed
                if initial_device.is_default != updated_device.is_default {
                    println!(
                        "Device {} default status changed: {} -> {}",
