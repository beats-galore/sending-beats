...
mod audio_callback_tests {
    ...
    use cpal::{SampleFormat, StreamConfig, SampleRate};
    ...
    async fn test_callback_format_handling() {
        ...
        let converted_f32: Vec<f32> = i16_data.iter()
    ...
    async fn test_high_frequency_callbacks() {
        ...
        for _ in 0..iterations {
            ...
            }).await.expect("High frequency callback failed");
        }
        ...
        assert!(elapsed < Duration::from_secs(5), "Callbacks took too long: {:?}", elapsed);
    }
