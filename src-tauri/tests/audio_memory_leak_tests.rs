...
mod audio_memory_leak_tests {
    ...
    async fn test_audio_buffer_memory_leaks() {
        ...
        for buffer_size in buffer_sizes {
            ...
            println!(
                "Buffer size {}: {} allocs, {} deallocs, peak {} bytes, current {} bytes",
            ...
            assert_eq!(
                allocs, deallocs,
                "Allocations and deallocations should match for buffer size {}",
                buffer_size
            );
            assert_eq!(
                current, 0,
                "Current memory usage should be zero after cleanup for buffer size {}",
                buffer_size
            );
        }
    ...
    async fn test_audio_stream_memory_leaks() {
        ...
        assert!(
            ...
            "Memory growth ({} KB) suggests potential memory leak",
            memory_growth / 1024
        );
    ...
    async fn test_concurrent_processing_memory_leaks() {
        ...
        assert_eq!(
            allocs, deallocs,
            "Allocations and deallocations should match in concurrent processing"
        );
        assert_eq!(
            current, 0,
            "Current memory usage should be zero after concurrent processing"
        );
    ...
    async fn test_device_enumeration_memory_leaks() {
        ...
        assert!(
            memory_growth < 20 * 1024 * 1024, // 20MB threshold (more realistic for audio device probing)
    ...
    async fn test_audio_effects_memory_leaks() {
        ...
        for i in 0..iterations {
            ...
            // Simulate complex audio effects chain
            {
                ...
                // EQ processing
                let eq_buffer: Vec<f32> = input_buffer
                    .iter()
                    .map(|&sample| {
                        let low = sample * 1.0;
                        let mid = sample * 1.1;
                ...
                let comp_buffer: Vec<f32> = eq_buffer
                    .iter()
                    .map(|&sample| {
                        let threshold = 0.7;
                        let ratio = 4.0;
                ...
                // Limiter processing
                let _limited_buffer: Vec<f32> = comp_buffer
                    .iter()
                    .map(|&sample| {
                        let limit = 0.95;
                        if sample.abs() > limit {
        ...
        assert_eq!(
            allocs, deallocs,
            "Allocations and deallocations should match in effects processing"
        );
        assert_eq!(
            current, 0,
            "Current memory usage should be zero after effects processing"
        );
    ...
    async fn test_long_running_memory_stability() {
        ...
        if !memory_samples.is_empty() {
            ...
            assert!(
                memory_variance < 50 * 1024 * 1024, // 50MB variance threshold
            ...
            assert!(
                final_growth < 20 * 1024 * 1024, // 20MB growth threshold
    ...
    async fn test_audio_resource_cleanup() {
        ...
        // Test async resource cleanup
        {
            ...
            for i in 0..5 {
                ...
                .await
                .expect("Async resource creation failed");
            }
