...
mod audio_performance_benchmarks {
    ...
    async fn benchmark_audio_buffer_processing() {
        ...
        for buffer_size in buffer_sizes {
            ...
            while start_time.elapsed() < test_duration {
                ...
                let processed_buffer: Vec<f32> = input_buffer.iter()
                    .map(|&sample| {
            ...
            let cpu_usage_percent = (real_time_samples_per_sec as f64 / samples_per_second as f64) * 100.0;
            
            println!("{:>10} | {:>13} μs | {:>12} | {:>6.2}%", 
                     buffer_size, avg_processing_time, samples_per_second, cpu_usage_percent);
            
            // Performance assertions
            assert!(samples_per_second > real_time_samples_per_sec, 
                   "Processing should be faster than real-time for buffer size {}", buffer_size);
            assert!(cpu_usage_percent < 50.0, 
                   "CPU usage should be reasonable (<50%) for buffer size {}", buffer_size);
        }
    }
    ...
    async fn benchmark_audio_level_calculation() {
        ...
        for buffer_size in buffer_sizes {
            ...
            assert!(combined_per_iter < 25000, "Combined calculation should be under 25μs");
        }
    }
    ...
    async fn benchmark_concurrent_audio_processing() {
        ...
        for channel_count in channel_counts {
            ...
            for channel_id in 0..channel_count {
                ...
                let handle = tokio::spawn(async move {
                    ...
                    while start_time.elapsed() < test_duration {
                        ...
                        let processed: Vec<f32> = input_buffer.iter()
                            .map(|&sample| {
            ...
            let efficiency = (samples_per_second as f64 / (single_channel_baseline * channel_count as u64) as f64) * 100.0;
            
            println!("{:>7} | {:>13} | {:>11} | {:>9.1}%", 
                     channel_count, total_samples, samples_per_second, efficiency);
            
            // Performance assertions
            assert!(samples_per_second > 44100 * channel_count as u64, 
                   "Should process faster than real-time for {} channels", channel_count);
        }
    }
    ...
    async fn benchmark_memory_allocation() {
        ...
        for size in allocation_sizes {
            ...
            println!("{:>13} | {:>13} ns | {:>15} ns", 
                     size, alloc_per_iter, dealloc_per_iter);
            
            // Performance assertions
            assert!(alloc_per_iter < 100000, "Allocation should be under 100μs for {} samples", size);
            assert!(dealloc_per_iter < 50000, "Deallocation should be under 50μs for {} samples", size);
        }
    }
    ...
    async fn benchmark_format_conversion() {
        ...
        let start = Instant::now();
        for _ in 0..iterations {
            let converted: Vec<f32> = i16_buffer.iter()
        ...
        let f32_buffer: Vec<f32> = (0..buffer_size)
            .map(|i| (i as f32 * 0.001).sin())
            .collect();
        
        let start = Instant::now();
        for _ in 0..iterations {
            let converted: Vec<i16> = f32_buffer.iter()
        ...
        let start = Instant::now();
        for _ in 0..iterations {
            let stereo: Vec<f32> = mono_left.iter()
                .zip(mono_right.iter())
                .flat_map(|(&l, &r)| [l, r])
                .collect();
        ...
        let i16_samples_per_sec = (buffer_size as u64 * iterations as u64) as f64 / i16_to_f32_duration.as_secs_f64();
        let f32_samples_per_sec = (buffer_size as u64 * iterations as u64) as f64 / f32_to_i16_duration.as_secs_f64();
        let stereo_samples_per_sec = (buffer_size as u64 * iterations as u64) as f64 / stereo_interleave_duration.as_secs_f64();
        
        println!("{:>14} | {:>10.2} μs | {:>11.0}", 
                 "i16 → f32", i16_to_f32_duration.as_micros() as f64 / iterations as f64, i16_samples_per_sec);
        println!("{:>14} | {:>10.2} μs | {:>11.0}", 
                 "f32 → i16", f32_to_i16_duration.as_micros() as f64 / iterations as f64, f32_samples_per_sec);
        println!("{:>14} | {:>10.2} μs | {:>11.0}", 
                 "Stereo Mix", stereo_interleave_duration.as_micros() as f64 / iterations as f64, stereo_samples_per_sec);
        
        // Performance assertions
        assert!(i16_samples_per_sec > 44100.0 * 100.0, "i16→f32 conversion should be very fast");
        assert!(f32_samples_per_sec > 44100.0 * 100.0, "f32→i16 conversion should be very fast");
        assert!(stereo_samples_per_sec > 44100.0 * 50.0, "Stereo interleaving should be fast");
    }
    ...
    async fn performance_regression_detection() {
        ...
        for baseline in baselines {
            println!("✓ {}: min {} samples/sec, max {:.1}ms latency", 
