...
impl TccPermissionManager {
    ...
    async fn check_via_audio_device_access(&self) -> TccPermissionStatus {
        ...
        let devices = match cpal::default_host().input_devices() {
            ...
            Err(e) => {
                error!(
                    "Failed to enumerate audio devices: {} - permissions likely denied",
                    e
                );
        ...
        let device = match cpal::default_host().default_input_device() {
            ...
            None => {
                warn!(
                    "No default input device found - permissions may be denied or no mic available"
                );
        ...
        let config = match device.default_input_config() {
            ...
            Err(e) => {
                error!(
                    "Failed to get default input config: {} - permissions likely denied",
                    e
                );
                return TccPermissionStatus::Denied;
            }
        ...
        match stream_result {
            ...
            Err(e) => {
                    ...
                    || error_str.contains("denied")
                {
                    ...
                    warn!(
                        "Unknown error creating audio stream: {} - assuming not determined",
                        e
                    );
