...
pub fn validate_device_id(device_id: &str) -> Result<()> {
    ...
    if device_id.len() > 256 {
        return Err(anyhow::anyhow!(
            "Device ID too long: maximum 256 characters allowed, got {}",
            device_id.len()
        ));
    }
    if device_id.len() < 2 {
        return Err(anyhow::anyhow!(
            "Device ID too short: minimum 2 characters required"
        ));
    }
    ...
    if !device_id.chars().all(valid_chars) {
        let invalid_chars: String = device_id
            .chars()
    ...
    if device_id.starts_with(|c: char| !c.is_alphanumeric()) {
        return Err(anyhow::anyhow!(
            "Device ID must start with alphanumeric character"
        ));
    }
    if device_id.ends_with(|c: char| !c.is_alphanumeric()) {
        return Err(anyhow::anyhow!(
            "Device ID must end with alphanumeric character"
        ));
    }
    ...
    for pattern in &dangerous_patterns {
        if device_id.contains(pattern) {
            return Err(anyhow::anyhow!(
                "Device ID contains dangerous pattern: '{}'",
                pattern
            ));
        }
    }
...
pub fn validate_config(config: &MixerConfig) -> Result<()> {
    ...
    if config.sample_rate < 8000 || config.sample_rate > 192000 {
        return Err(anyhow::anyhow!(
            "Invalid sample rate: {} (must be 8000-192000 Hz)",
            config.sample_rate
        ));
    }
    ...
    if config.buffer_size < 16 || config.buffer_size > 8192 {
        return Err(anyhow::anyhow!(
            "Invalid buffer size: {} (must be 16-8192 samples)",
            config.buffer_size
        ));
    }
    ...
    if config.channels.len() > 32 {
        return Err(anyhow::anyhow!(
            "Invalid channel count: {} (must be 0-32 channels)",
            config.channels.len()
        ));
    }
...
pub fn validate_channel_id(channel_id: u32) -> Result<()> {
    ...
    if channel_id > 9999 {
        return Err(anyhow::anyhow!(
            "Channel ID too large: {} (maximum: 9999)",
            channel_id
        ));
    }
    Ok(())
...
pub fn validate_audio_levels(
    ...
    for (level, name) in levels.iter().zip(names.iter()) {
        ...
        if *level < 0.0 {
            return Err(anyhow::anyhow!(
                "Audio level {} cannot be negative: {}",
                name,
                level
            ));
        }
        if *level > 100.0 {
            return Err(anyhow::anyhow!(
                "Audio level {} is too large: {} (maximum: 100.0)",
                name,
                level
            ));
        }
    ...
    if rms_left > peak_left {
        return Err(anyhow::anyhow!(
            "RMS left ({}) cannot exceed peak left ({})",
            rms_left,
            peak_left
        ));
    }
    if rms_right > peak_right {
        return Err(anyhow::anyhow!(
            "RMS right ({}) cannot exceed peak right ({})",
            rms_right,
            peak_right
        ));
    }
