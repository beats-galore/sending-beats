...
pub trait AudioWorker {
    ...
    /// Update adaptive chunk size when sample rate changes
    #[cfg(target_os = "macos")]
    fn update_adaptive_chunk_size(
        ...
        let optimal_chunk_size =
            calculate_optimal_chunk_size(device_sample_rate, target_sample_rate, base_chunk_size);

        if optimal_chunk_size != current_chunk_size {
            info!(
                "🔧 {}: {} updated chunk size to {} for {}Hz→{}Hz",
                "DYNAMIC_CHUNKS".on_cyan().white(),
                device_id,
                optimal_chunk_size,
                device_sample_rate,
                target_sample_rate
            ...
            // Update CoreAudio hardware buffer size to match
            if let Some(ref hardware_tx) = hardware_update_tx {
                ...
                if let Err(e) = hardware_tx.try_send(command) {
                    ...
                    info!(
                        ...
                        "HARDWARE_SYNC_COMMAND".on_cyan().white(),
                        optimal_chunk_size
                    );
                }
            }
    ...
    /// Update adaptive chunk size when sample rate changes (non-macOS)
