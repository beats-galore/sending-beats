// Sample-level mixing utilities shared by the input workers and the bus mixer.
//
// These were the last inhabitants of the old VirtualMixer; the struct itself
// was an empty shell from a previous pipeline generation and is gone.
...
pub fn mix_input_samples_ref(input_samples: &[(&str, &[f32])]) -> Vec<f32> {
    ...
    static BUFFER_DEBUG_COUNT: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
    ...
    REUSABLE_MIX_BUFFER.with(|buf| {
        ...
        for (_device_id, samples) in input_samples.iter() {
            if !samples.is_empty() {
                active_channels += 1;
        ...
        if active_channels > 1 && ENABLE_AUTO_GAIN_REDUCTION {
            apply_auto_gain_reduction(&mut buffer, active_channels);
        }
