...
use super::super::types::{
    AudioDeviceHandle, AudioMetrics, MixerCommand, MixerConfig, OutputDevice,
};
...
pub struct VirtualMixer {
    ...
    pub audio_command_tx:
        tokio::sync::mpsc::Sender<crate::audio::mixer::stream_management::AudioCommand>, // Command channel to isolated audio thread
...
impl VirtualMixer {
    ...
    pub async fn new_with_device_manager(
        ...
        audio_command_tx: tokio::sync::mpsc::Sender<
            crate::audio::mixer::stream_management::AudioCommand,
        >,
    ) -> anyhow::Result<Self> {
    ...
    pub async fn get_all_device_health_statuses(
        &self,
    ) -> std::collections::HashMap<String, crate::audio::devices::DeviceHealth> {
    ...
    pub async fn add_input_stream_safe(&self, device_id: &str) -> anyhow::Result<()> {
        ...
        info!(
            "Adding input stream for device with health checking: {}",
            device_id
        );
            ...
            .await
        {
            Ok(DeviceStatus::Connected) => {
                ...
                println!(
                    "✅ Device {} is healthy, proceeding with stream creation",
                    device_id
                );
                Ok(())
            }
            Ok(DeviceStatus::Disconnected) => {
                self.audio_device_manager
                    .report_device_error(device_id, "Device disconnected".to_string())
                    .await;
            ...
            Err(e) => {
                self.audio_device_manager
                    .report_device_error(device_id, format!("Health check failed: {}", e))
                    .await;
                return Err(anyhow::anyhow!(
    ...
    pub async fn get_device_health_status(
        &self,
        device_id: &str,
    ) -> Option<crate::audio::devices::DeviceHealth> {
    ...
    pub fn get_channel_mut(
        &mut self,
        channel_id: u32,
    ) -> Option<&mut crate::audio::types::AudioChannel> {
    ...
    pub async fn add_output_device(
        ...
        println!(
            "✅ Added output device: {} ({})",
            output_device.device_name, output_device.device_id
        );
        Ok(())
    }
    ...
    pub async fn remove_output_device(&self, device_id: &str) -> anyhow::Result<()> {
        ...
        let removed = self.active_output_devices.lock().await.remove(device_id);
    ...
    pub async fn update_output_device(
        &self,
        device_id: &str,
        updated_device: super::types::OutputDevice,
    ) -> anyhow::Result<()> {
        // Update config
        {
            let mut config_guard = self.shared_config.lock().unwrap();
            if let Some(device) = config_guard
                .output_devices
                .iter_mut()
                .find(|d| d.device_id == device_id)
            {
                *device = updated_device;
                ...
                Err(anyhow::anyhow!(
                    "Output device not found in config: {}",
                    device_id
                ))
            }
        }
