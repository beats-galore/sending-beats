...
use super::silence_detection::{AudioQualityAnalyzer, SilenceDetector};
use super::types::{
    RecordingConfig, RecordingFormat, RecordingHistoryEntry, RecordingMetadata, RecordingSession,
    RecordingStatus,
};
...
impl RecordingWriter {
    ...
    pub async fn new(config: RecordingConfig) -> Result<Self> {
        ...
        info!(
            "Created recording writer for: {}",
            session.current_file_path.display()
        );
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        let file = File::create(&write_path).await.with_context(|| {
            format!("Failed to create recording file: {}", write_path.display())
        })?;
        ...
        self.session
            .metadata
            .set_technical_metadata(&self.session.config, &encoder_name);

        info!(
            "Started recording to temporary file: {}",
            write_path.display()
        );
        Ok(())
    }
    ...
    pub async fn process_samples(&mut self, samples: &[f32]) -> Result<bool> {
        ...
        self.session.duration_seconds =
            self.samples_processed as f64 / sample_rate / self.session.config.channels as f64;
        ...
        {
            info!(
                "📊 Session {} stats: duration {:.1}s (was {:.1}s), samples {}, file_size {}B",
                self.session.id,
                self.session.duration_seconds,
                old_duration,
        ...
        if !quality.is_acceptable() {
            warn!(
                "Poor audio quality detected: {} ({}% clipping)",
                quality.get_quality_text(),
                quality.clip_rate_percent
            );
        }
        ...
        if let Some(detector) = &mut self.silence_detector {
            ...
            if analysis.should_auto_stop {
                info!(
                    "Auto-stopping recording due to silence: {:.1}s",
                    analysis.silence_duration_seconds()
                );
                should_stop = true;
            }
        }
        ...
        if self.session.should_auto_stop_duration() {
            info!(
                "Auto-stopping recording due to duration limit: {:.1}min",
        ...
        if self.session.should_auto_stop_size() {
            info!(
                "Auto-stopping recording due to file size limit: {}MB",
    ...
    pub async fn stop(&mut self) -> Result<RecordingHistoryEntry> {
        ...
        self.session
            .metadata
            .set_duration(self.session.duration_seconds);
        ...
        info!(
            "Recording completed: {} ({} samples, {:.1}s, {} bytes)",
...
impl RecordingWriterManager {
    ...
    pub async fn initialize(&self) -> Result<Vec<String>> {
        ...
        info!(
            "Recording writer manager initialized, recovered {} temp files",
            recovered_files.len()
        );
        Ok(recovered_files)
    }
            ...
            if let Some(extension) = path.extension() {
                if extension == "tmp" {
                    ...
                    if let Some(stem) = path.file_stem() {
                        ...
                        if stem_str.ends_with(".wav")
                            || stem_str.ends_with(".mp3")
                            || stem_str.ends_with(".flac")
                        {
                            // Try to recover this file
                            match self.attempt_recovery(&path).await {
                                Ok(final_path) => {
                                    info!(
                                        "Recovered temporary file: {} -> {}",
                                        path.display(),
                                        final_path
                                    );
    ...
    async fn attempt_recovery(&self, temp_path: &PathBuf) -> Result<String> {
        ...
        let unique_final_path =
            super::filename_generation::PathManager::make_unique_filename(&final_path);
            ...
            Ok(writers) => {
                ...
                let available_space_gb = if let Some((_, writer)) = writers.iter().next() {
                    ...
                    if let Ok(writer_guard) = writer.try_lock() {
                        let recording_dir = writer_guard
                            .session
                            .current_file_path
                            .parent()
                            .unwrap_or_else(|| std::path::Path::new("."));

                        match super::filename_generation::PathManager::get_available_space(
                            recording_dir,
                        ) {
