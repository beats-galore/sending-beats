...
impl RecordingService {
                    ...
                    RecordingCommand::Stop => {
                        ...
                        if let Some(session_id) = session_id {
                            match writer_manager.stop_recording(&session_id).await {
                                Ok(history_entry) => {
                                    info!("Stopped recording session: {} - {}", 
                                          session_id, history_entry.get_duration_display());
                                }
                                Err(e) => {
    ...
    pub async fn process_audio_samples(&self, samples: &[f32]) -> Result<()> {
        ...
        if let Some(session_id) = session_id {
            let should_continue = self.writer_manager.process_samples(&session_id, samples).await?;
    ...
    pub fn create_from_preset(&self, preset_name: &str, custom_name: Option<String>) -> Result<RecordingConfig> {
    ...
    pub async fn get_statistics(&self) -> Result<RecordingStatistics> {
        ...
        let formats_used: HashMap<String, usize> = history.iter()
...
mod tests {
    ...
    fn test_preset_creation() {
        ...
        assert!(presets.iter().any(|(name, _)| *name == "High Quality Stereo"));
    }
    ...
    fn test_filename_templates() {
        ...
        assert!(templates.iter().any(|(_, template)| template.contains("{timestamp}")));
    }
}