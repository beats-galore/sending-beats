...
impl TemplateVariables {
    ...
    pub fn from_config(config: &RecordingConfig) -> Self {
        ...
        variables.insert("format".to_string(), config.format.get_format_name().to_lowercase());
        variables.insert("sample_rate".to_string(), config.sample_rate.to_string());
        variables.insert("channels".to_string(), if config.channels == 1 { "mono" } else { "stereo" }.to_string());
        variables.insert("bit_depth".to_string(), config.bit_depth.to_string());
        
        // Metadata (with fallbacks)
        variables.insert("title".to_string(), 
            config.metadata.title.as_ref()
                .map(|t| sanitize_filename(t))
                .unwrap_or_else(|| "recording".to_string())
        );
        variables.insert("artist".to_string(), 
            config.metadata.artist.as_ref()
                .map(|a| sanitize_filename(a))
                .unwrap_or_else(|| "unknown_artist".to_string())
        );
        variables.insert("album".to_string(), 
            config.metadata.album.as_ref()
                .map(|a| sanitize_filename(a))
                .unwrap_or_else(|| "unknown_album".to_string())
        );
        variables.insert("genre".to_string(), 
            config.metadata.genre.as_ref()
    ...
    pub fn set(&mut self, key: &str, value: String) {
        self.variables.insert(key.to_string(), sanitize_filename(&value));
    }
...
impl FilenameGenerator {
    ...
    pub fn generate_with_variables(&self, template: &str, variables: &TemplateVariables) -> Result<String> {
        ...
        filename = self.template_regex.replace_all(&filename, |caps: &regex::Captures| {
            let var_name = &caps[1];
        ...
        info!("Generated filename: {} from template: {}", filename, template);
        Ok(filename)
    }
    ...
    pub fn validate_template(&self, template: &str) -> Result<Vec<String>> {
        ...
        for pattern in &dangerous_patterns {
            if template.contains(pattern) {
                return Err(anyhow::anyhow!("Template contains dangerous pattern: '{}'", pattern));
            }
        }
        
        // Extract all template variables
        let variables: Vec<String> = self.template_regex
...
impl PathManager {
    ...
    pub fn is_safe_recording_path(path: &Path) -> bool {
        ...
        let dangerous_paths = [
            "/system", "/boot", "/usr", "/bin", "/sbin", "/etc", "/dev", "/proc", "/sys",
...
mod tests {
    ...
    fn test_filename_sanitization() {
        ...
        assert_eq!(sanitize_filename("with/slash\\backslash"), "with_slash_backslash");
    ...
    fn test_template_variables() {
        ...
        let variables = TemplateVariables::from_config(&config);
        assert_eq!(variables.get("config_name"), Some(&"Test_Config".to_string()));
    ...
    #[test]
    fn test_path_safety() {
        assert!(PathManager::is_safe_recording_path(Path::new("/Users/test/Music")));
        assert!(!PathManager::is_safe_recording_path(Path::new("/system")));
        assert!(!PathManager::is_safe_recording_path(Path::new("relative/path")));
    }
}