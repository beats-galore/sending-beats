...
#[cfg(target_os = "macos")]
use coreaudio_sys::{
    AudioDeviceID, kAudioHardwarePropertyDevices, kAudioObjectPropertyScopeGlobal, 
    kAudioObjectPropertyElementMaster, kAudioDevicePropertyDeviceNameCFString, 
...
#[cfg(target_os = "macos")]
use crate::audio::types::{AudioDeviceInfo, AudioDeviceHandle, CoreAudioDevice};
...
impl CoreAudioIntegration {
            ...
            match self.get_coreaudio_device_info(device_id).await {
                Ok(device_infos) => {
                    for device_info in device_infos {
                        info!("Found CoreAudio device: {} ({})", device_info.name, device_info.id);
    ...
    async fn device_has_streams(&self, device_id: AudioDeviceID, is_input: bool) -> Result<bool> {
        ...
        let streams_property = AudioObjectPropertyAddress {
            ...
            mScope: if is_input { kAudioObjectPropertyScopeInput } else { kAudioObjectPropertyScopeOutput },
            mElement: kAudioObjectPropertyElementMaster,
        };
    ...
    pub async fn is_coreaudio_default_device(&self, device_id: AudioDeviceID, is_output: bool) -> Result<bool> {
        use std::mem;
        use std::ptr;
    ...
    pub async fn create_coreaudio_device_handle(&self, device_info: &AudioDeviceInfo, _is_input: bool) -> Result<AudioDeviceHandle> {
        ...
        match self.find_coreaudio_device_id(&device_info.name).await {
            Ok(device_id) => {
                info!("Creating CoreAudio handle for device {} (ID: {})", device_info.name, device_id);
            ...
            Err(e) => {
                warn!("Failed to find CoreAudio device ID for {}: {}", device_info.name, e);
                Err(e)
            }
        }
    }
...
#[cfg(not(target_os = "macos"))]
impl CoreAudioIntegration {
    pub fn new(_devices_cache: std::sync::Arc<tokio::sync::Mutex<std::collections::HashMap<String, crate::audio::types::AudioDeviceInfo>>>) -> Self {
        Self
    }

    pub async fn enumerate_coreaudio_devices(&self) -> anyhow::Result<Vec<crate::audio::types::AudioDeviceInfo>> {
        Ok(Vec::new())
    }

    pub async fn create_coreaudio_device_handle(&self, _device_info: &crate::audio::types::AudioDeviceInfo, _is_input: bool) -> anyhow::Result<crate::audio::types::AudioDeviceHandle> {
