...
impl AudioDeviceManager {
    ...
    pub async fn check_device_health(&self, device_id: &str) -> Result<DeviceStatus> {
        ...
        self.health_monitor.check_device_health(device_id, device_exists).await
    }
    
    /// Report a device error and update health tracking
    pub async fn report_device_error(&self, device_id: &str, error: String) {
        self.health_monitor.report_device_error(device_id, error).await;
    }
    ...
    pub async fn find_audio_device(&self, device_id: &str, is_input: bool) -> Result<AudioDeviceHandle> {
        info!("Searching for audio device: {} (input: {})", device_id, is_input);
    ...
    pub async fn find_cpal_device(&self, device_id: &str, is_input: bool) -> Result<Device> {
        info!("🔍 CPAL DEVICE SEARCH: Looking for {} device: {}", 
        ...
        if let Some(info) = device_info {
            info!("📋 CACHE HIT: Found device info: {} -> {}", device_id, info.name);
        }
    ...
    async fn get_system_default_device(&self, is_input: bool) -> Result<Device> {
        info!("🆘 SYSTEM DEFAULT: Attempting to use system default {} device", 
            if is_input { "input" } else { "output" });
    ...
    pub async fn initialize_device_health(&self, device_info: &AudioDeviceInfo) {
        self.health_monitor.initialize_device_health(device_info).await;
    }
