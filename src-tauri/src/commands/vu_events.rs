...
pub async fn initialize_vu_channels(
    ...
    tracing::info!(
        "{}: initialize_vu_channels command called",
        "VU_CHANNELS_INIT".bright_green()
    );
    ...
    tracing::info!(
        "{}: Sending SetVUChannel command to audio system...",
        "VU_CHANNELS_INIT".bright_green()
    );
    ...
    if let Err(_) = audio_state.audio_command_tx.send(command).await {
        tracing::error!(
            "{}: Failed to send VU channel command to audio system",
            "VU_CHANNELS_ERROR".red()
        );
        return Err("Failed to send VU channel to audio system".to_string());
    }

    tracing::info!(
        "{}: VU channel command sent, waiting for response...",
        "VU_CHANNELS_INIT".bright_green()
    );
    ...
    match response_rx.await {
        Ok(Ok(())) => {
            tracing::info!(
                "{}: VU channels initialized successfully",
                "VU_CHANNELS_SUCCESS".bright_green()
            );
            Ok(())
        }
        Ok(Err(e)) => {
            tracing::error!(
                "{}: Audio system reported error: {}",
                "VU_CHANNELS_ERROR".red(),
                e
            );
