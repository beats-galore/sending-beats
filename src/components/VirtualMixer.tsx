...
const VUMeter: React.FC<{
  ...
  React.useEffect(() => {
    if (peakLevel > 0 || rmsLevel > 0) {
      console.log(`📶 VUMeter update: peak=${peakLevel.toFixed(3)} (${dbPeak.toFixed(1)}dB), rms=${rmsLevel.toFixed(3)} (${dbRms.toFixed(1)}dB), positions=${peakPosition.toFixed(2)}/${rmsPosition.toFixed(2)}`);
    }
      ...
      {Array.from({ length: segments }, (_, i) => {
        ...
        return (
          <div
            ...
            style={vertical ? { height: `${segmentSize}px` } : { width: `${segmentSize}px`, height: '4px' }}
          />
        );
      })}
    </div>
  ...
  return (
    <div className="bg-surface-light p-4 rounded-lg border border-surface">
      ...
      <div className="grid grid-cols-6 gap-4 mb-3">
        ...
        <div>
          <label className="block text-xs text-surface-light mb-1">
            Pan: {channel.pan > 0 ? `R${(channel.pan * 100).toFixed(0)}` : channel.pan < 0 ? `L${Math.abs(channel.pan * 100).toFixed(0)}` : 'Center'}
          </label>
          <input
            type="range"
            min="-1"
        ...
        <div>
          ...
          <input
            ...
            className="w-full"
          />
          <div className="text-xs text-center">{channel.eq_mid_gain.toFixed(1)}/{channel.eq_low_gain.toFixed(1)}</div>
        </div>
      </div>
        ...
        <div className="col-span-2">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              className={`px-2 py-1 text-xs font-medium rounded ${
                channel.comp_enabled ? 'bg-green-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500'
              }`}
            >
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              className={`px-2 py-1 text-xs font-medium rounded ${
                channel.limiter_enabled ? 'bg-orange-600 text-white' : 'bg-gray-600 text-gray-300 hover:bg-gray-500'
              }`}
            >
...
const VirtualMixer: React.FC = () => {
  ...
  const [masterLevels, setMasterLevels] = useState<{ leftPeak: number, leftRms: number, rightPeak: number, rightRms: number }>({
  ...
  useEffect(() => {
    if (isRunning) {
      const pollMetrics = async () => {
        try {
          console.log("🔍 Polling VU meter data...");
          const currentMetrics = await invoke<AudioMetrics>("get_mixer_metrics");
          setMetrics(currentMetrics);
          
          // Get real-time channel levels for VU meters
          const channelLevels = await invoke<Record<number, [number, number]>>("get_channel_levels");
          console.log("📊 Received channel levels:", channelLevels);
          
          // Get real-time master levels for VU meters
          const masterLevelsData = await invoke<[number, number, number, number]>("get_master_levels");
          console.log("📊 Received master levels:", masterLevelsData);
          setMasterLevels({
          ...
          setMixerConfig(prev => {
            ...
            return {
              ...
              channels: prev.channels.map(channel => {
                ...
                if (levels) {
                  console.log(`🎚️ Channel ${channel.id} levels: peak=${levels[0].toFixed(3)}, rms=${levels[1].toFixed(3)}`);
                  return {
                    ...channel,
                    peak_level: levels[0],
                    rms_level: levels[1]
      ...
      return () => {
        if (metricsIntervalRef.current) {
          console.log("🛑 Stopping VU meter polling...");
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                >
                  ...
                  {audioDevices.filter(device => device.is_output).map(device => (
                    <option key={device.id} value={device.id}>
