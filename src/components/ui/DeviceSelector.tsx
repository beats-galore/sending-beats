...
export const DeviceSelector = memo<DeviceSelectorProps>(
    ...
    return (
      <div className="w-full">
        ...
        >
          ...
          {/* Device options */}
          {devices.map((device) => (
            <option key={device.id} value={device.id} className="bg-gray-800 text-gray-200">
              {device.name}
        ...
        {selectedDeviceId && !selectedDevice && (
          <div className="mt-1 text-xs text-red-400">⚠️ Selected device not found</div>
        )}
      </div>
    );
  },
  (prevProps, nextProps) =>
    ...
    prevProps.devices.every(
      (device, index) =>
        device.id === nextProps.devices[index]?.id && device.name === nextProps.devices[index]?.name
    )
);
