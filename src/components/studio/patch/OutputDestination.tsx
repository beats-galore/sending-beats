...
export const OutputDestination = ({
  ...
  <NodeCard
    ...
    borderColor={output.live ? color.line : color.dash}
    dimmed={!output.live}
    ports={<PortDot tone={output.live ? ROLE_PORT[output.role] : 'dead'} side="left" top={24} />}
    header={
      <>
        <StatusDot
          tone={output.live ? 'accent' : 'inert'}
          onClick={() => onSelect(output.id)}
          title={output.live ? 'Receiving the master sum' : 'Send the master sum here'}
        />
        <Text
