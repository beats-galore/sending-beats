...
export const useMixerState = () => {
  ...
  const updateChannelGain = useCallback(async (channelId: number, gain: number) => {
    await updateChannel(channelId, { gain });
  }, [updateChannel]);

  const updateChannelPan = useCallback(async (channelId: number, pan: number) => {
    await updateChannel(channelId, { pan });
  }, [updateChannel]);

  const toggleChannelMute = useCallback(async (channelId: number) => {
  ...
  const toggleChannelSolo = useCallback(async (channelId: number) => {
    const channel = getChannelById(channelId);
    if (channel) {
  ...
  const setChannelInputDevice = useCallback(async (channelId: number, deviceId: string) => {
    await updateChannel(channelId, { input_device_id: deviceId });
  }, [updateChannel]);

  // EQ controls
  const updateChannelEQ = useCallback(async (channelId: number, eq: {
    eq_low_gain?: number;
    eq_mid_gain?: number;
    eq_high_gain?: number;
  }) => {
  ...
  // Compressor controls
  const updateChannelCompressor = useCallback(async (channelId: number, comp: {
  ...
  // Limiter controls
  const updateChannelLimiter = useCallback(async (channelId: number, limiter: {
  ...
  // Master controls
  const setMasterGain = useCallback(async (gain: number) => {
