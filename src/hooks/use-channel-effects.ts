...
export const useChannelEffects = (channelId: number) => {
  ...
  // EQ controls
  const setEQLowGain = useCallback(async (gain: number) => {
  ...
  const setEQMidGain = useCallback(async (gain: number) => {
    const clampedGain = audioCalculations.clampGain(gain);
    await updateChannelEQ(channelId, { eq_mid_gain: clampedGain });
  }, [channelId, updateChannelEQ]);

  const setEQHighGain = useCallback(async (gain: number) => {
    const clampedGain = audioCalculations.clampGain(gain);
  ...
  // Compressor controls
  const setCompressorThreshold = useCallback(async (threshold: number) => {
    const clampedThreshold = Math.max(AUDIO.COMP_MIN_THRESHOLD, Math.min(AUDIO.COMP_MAX_THRESHOLD, threshold));
    await updateChannelCompressor(channelId, { comp_threshold: clampedThreshold });
  }, [channelId, updateChannelCompressor]);

  const setCompressorRatio = useCallback(async (ratio: number) => {
  ...
  const setCompressorAttack = useCallback(async (attack: number) => {
    const clampedAttack = Math.max(AUDIO.COMP_MIN_ATTACK, Math.min(AUDIO.COMP_MAX_ATTACK, attack));
    await updateChannelCompressor(channelId, { comp_attack: clampedAttack });
  }, [channelId, updateChannelCompressor]);

  const setCompressorRelease = useCallback(async (release: number) => {
    const clampedRelease = Math.max(AUDIO.COMP_MIN_RELEASE, Math.min(AUDIO.COMP_MAX_RELEASE, release));
  ...
  const setLimiterThreshold = useCallback(async (threshold: number) => {
    const clampedThreshold = Math.max(AUDIO.LIMITER_MIN_THRESHOLD, Math.min(AUDIO.LIMITER_MAX_THRESHOLD, threshold));
